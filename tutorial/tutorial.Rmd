...
The [Go programming language]() is the language after which the V languange
was designed. It as well supports easy integration for C code. In the code
the user places an include statement just before the `import "C"` statement.
Calls the C functions are then prefixed with the `C` letter.
Here again the example were we use our hello library in a file run-hello.go.

`include ../samples/hello/go/run-hello.go pre`

the sequence of commands to compile an the output then would look like this:
...
Optionally the variable `export CGO_ENABLED=1` must be set in your terminal
on older versions of the Go compiler. The cgo approach has a few drawbacks,
