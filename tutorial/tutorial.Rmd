...
by the compiler or interpeter. That handling can be vary different. We solve
...
but these settings can be overwritten.
...
We will now repeat the same with C++, which does currently not work properly
as in the main function _parseInt_ is not prefixed with _Main4::_.

```{.shell label=Main4 cmd="fut %i -o main4.cpp&&g++ -std=c++17 cc main4.cpp -o main4-cpp&&./main4-cpp > %b.txt" chunk.ext=fu ext=txt,echo=FALSE}
#INCLUDE "Main4.fu"
```

`include main4.cpp {<pre> </pre>}`

We can fix that by prefixing _parseInt_ with *'Main4::'* in the main function
similarly we have prefixed _parseInt_ in the C-code with *'Main4_'*.

> [!CAUTION]
> We should keep in mind that the Fusion programming language is not meant to be used to 
> create application on its own. The basic library is quite limited so to write applications
> you could better code directly in C++ or any other language you are comfortable with.
